...
function crea() {
    ...
    this.input.on('dragend', function (pointer, object, x, y) {
        ...
        // Verificar si el huevo cae en la huevera correcta
        if (Phaser.Geom.Intersects.RectangleToRectangle(huevera_b.getBounds(), object.getBounds())) {
            ...
            object.destroy(); // Eliminar el huevo
    ...
    countdown_text = this.add.text(field_center, 16, countdown, { "fontSize": 48, "fontStyle": "bold" });
    ...
    // Crear el texto de Game Over, pero ocultarlo inicialmente
    game_over_text = this.add.text(canvas_w / 2, canvas_h / 2, 'HAS PERDIDO', {
        fontSize: '64px',
        fontStyle: 'bold',
        fill: '#ffffff'
    }).setOrigin(0.5, 0.5);
    game_over_text.setVisible(false); // Inicia oculto
}
...
function actualiza() {
    ...
    // Verificar si el tiempo ha llegado a cero y mostrar el texto de Game Over
    if (countdown <= 0 && !game_over_text.visible) {
        console.log("Game Over");
        music.background.stop();
        music.game_over.play();
        game_over_text.setVisible(true);  // Mostrar el texto
